...
import com.madcode.blog.repositories.TagRepository;
...
public class TagServiceImpl implements TagService {
    ...
    public List<TagDto> getTags() {
        List<TagPostCountProjection> projections =
                tagRepository.findAllWithPostCount();

        return projections.stream()
                .map(p -> TagDto.builder()
                        .id(p.getId())
                        .name(p.getName())
                        .postCount((int) p.getPostCount())
                        .build())
                .toList();
    }
